...
const getAccounts = () => {
  return {
    cuenta: faker.number.bigInt(100000000000000000000, 999999999999999999999),
  }
}
...
(async () => {
  ...
  const customers = persons.map(person => {
    return { cliente_id: person.identificacion, nombre_cliente: `${person.nombre} ${person.apellido}` }
  })
  let accounts = []
  for (let i = 0; i < 400; i++) accounts.push(getAccounts())
  accounts = [...new Set(accounts.map(account => account.cuenta.toString().substring(0, 20)))];
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let i = 0; i < transfers.length; i++) {
      ...
      if (isOrigin || isDestination) {
        ...
        malla.push({
          ...
          "DOCUMENTO_ORIGEN": isOrigin ? person.identificacion : targetClient.identificacion,
          "CLIENTE_ORIGEN": isOrigin ? `${person.nombre} ${person.apellido}` : CLIENTE,
          "CUENTA_ORIGEN": isOrigin ? person.accounts[0] : targetAccountCode,
          "TIPO_DOCUMENTO": "V",
          "DOCUMENTO": isDestination ? person.identificacion : targetClient.identificacion,
          "CLIENTE_DESTINO": isDestination ? `${person.nombre} ${person.apellido}` : CLIENTE,
    ...
    XLSX.writeFile(createWorkbook(malla, `malla-${person.identificacion}`), `excels/malla-${person.identificacion}.xlsx`);
    const alerta = {
  ...
  for (let alerta of alertas) {
    for (let i = 0; i < 30; i++) {
      ...
      const accion = {
        ...
        "FECHA": faker.date.between({ from: '2024-01-01T00:00:00.000Z', to: '2024-06-01T00:00:00.000Z' }).toISOString()
      };
  ...
  XLSX.writeFile(createWorkbook(transfers, 'transfers'), 'excels/transfers.xlsx');
  XLSX.writeFile(createWorkbook(accountsPersons, 'accounts'), 'excels/accounts.xlsx');
  XLSX.writeFile(createWorkbook(customers, 'customers'), 'excels/customers.xlsx');
